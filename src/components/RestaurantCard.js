...
class RestaurantCard extends Component {
  ...
  _getRestaurants = ({ lat, lng }) => {
    fetch(
      ...
      .then(data => {
        let list = [];
        data.nearby_restaurants.map(result => list.push(result.restaurant));
              ...
              {suggestedList.map((item, index) => {
                return (
                  <div
                    ...
                    key={index + 1}
                    onMouseOver={() => this._updateActiveLink(index)}
                    onClick={() => this._handleSubmitCity(index)}
                  >
                    {item.title}
                  </div>
                );
            ...
            return (
              ...
              >
                <div className="d-flex w-100 justify-content-between">
                  ...
                  <small>{"$".repeat(restaurant.price_range)}</small>
                </div>
                <div className="d-flex w-100 justify-content-between">
                  <p className="mb-1">{restaurant.location.address}</p>
